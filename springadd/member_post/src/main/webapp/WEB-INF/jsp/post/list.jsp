	...
	<div class="wrap">
			 ...
			 <form>
				...
				<div class = "row my-3">
					<div class="col-2">
						...
						<option value="TCW" ${pageDto.cri.type =='TCW' ?'selected' : ''}>제목 + 내용 + 작성자</option>
						</select>		 	
					</div>
					<div class = "col-4">
    	...
    	<table class="table table-hover text-center" style="table-layout: fixed;">
	        ...
	        <c:forEach items="${posts}" var="p" >
		        <tr>
		            <td>${p.pno}</td>
		            <td class="text-truncate text-start "><a href="view?pno=${p.pno}&${pageDto.cri.qs2}" class="text-decoration-none">${p.title}
		            <c:if test="${p.attachFlag}"><i class="fa-solid fa-paperclip"></i></c:if>
		            </a></td>
		            <td>${p.writer }</td>
		            <td>${p.regdate}</td>
		            <td>${p.viewCount}</td>
